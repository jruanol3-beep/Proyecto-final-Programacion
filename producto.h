...
private:
    ...
    bool validarPrecio(double precio, string campo) {
        if (precio <= 0) {
            cout << "Error: " << campo << " debe ser mayor a 0." << endl;
            return false;
        }
        return true;
    }
...
public:
    Producto() { id_producto = 0; id_marca = 0; existencia = 0; precio_costo = 0; precio_venta = 0; }
    Producto(int id, string prod, int im, string desc, string img, double pc, double pv, int ex) {
        id_producto = id;
        producto = prod;
        id_marca = im;
        descripcion = desc;
    ...
    void crear() {
        ...
        if (existencia < 0) {
            cout << "Error: La existencia no puede ser negativa." << endl;
            return;
        }
        try {
            ConexionDB db;
            ...
            db.getSession()->sql(
                "INSERT INTO supermercado.productos (producto, id_marca, descripcion, imagen, precio_costo, precio_venta, existencia) VALUES (?, ?, ?, ?, ?, ?, ?)"
            ).bind(producto, id_marca, descripcion, imagen, precio_costo, precio_venta, existencia).execute();
    ...
    void leer() {
        try {
            ...
            auto resultado = db.getSession()->sql(
                "SELECT p.id_producto, p.producto, m.marca, p.precio_costo, p.precio_venta, p.existencia "
                "FROM supermercado.productos p "
                "INNER JOIN supermercado.marcas m ON p.id_marca = m.id_marca"
            ).execute();
            cout << "\n===== LISTA DE PRODUCTOS =====" << endl;
            cout << "ID\tPRODUCTO\t\tMARCA\t\tP.COSTO\t\tP.VENTA\t\tEXISTENCIA" << endl;
            cout << "--------------------------------------------------------------------------------" << endl;
    ...
    void actualizar() {
        ...
        try {
            ...
            if (!db.conectar()) return;
            db.getSession()->sql(
                "UPDATE supermercado.productos SET producto=?, id_marca=?, descripcion=?, imagen=?, precio_costo=?, precio_venta=?, existencia=? WHERE id_producto=?"
            ).bind(producto, id_marca, descripcion, imagen, precio_costo, precio_venta, existencia, id_producto).execute();
    ...
    void borrar() {
        ...
        try {
            ...
            if (!db.conectar()) return;
            db.getSession()->sql(
                "DELETE FROM supermercado.productos WHERE id_producto=?"
    ...
    bool buscarPorId(int id) {
        try {
            ...
            auto resultado = db.getSession()->sql(
                "SELECT p.id_producto, p.producto, m.marca, p.precio_venta, p.existencia "
                "FROM supermercado.productos p "
                "INNER JOIN supermercado.marcas m ON p.id_marca = m.id_marca "
                "WHERE p.id_producto = ?"
            ).bind(id).execute();
            ...
            if (resultado.count() > 0) {
                ...
                cout << "Producto: " << fila[1] << " | Marca: " << fila[2] << " | Precio: Q" << fila[3] << " | Existencia: " << fila[4] << endl;
                return true;
            }
